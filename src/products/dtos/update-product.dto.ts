...
export class UpdateProductDto {
    ...
    static validate(data: { [key: string]: any }): [string | undefined, UpdateProductDto | undefined] {
        ...
        if (supplier && !ValidatorsConfig.isMongoId(supplier)) return ["Supplier not valid", undefined];
        if (description && description.length < 4) return ["Description too short", undefined];

        return [undefined, new UpdateProductDto(name, +price, +stock, category, supplier, description)];
    }
}